  ...
  for (int i = 1; i <= gridSize; ++i) {
    for (int j = 1; j <= gridSize; ++j) {
      ...
      if (cell.getOccupants().size() > 1) {
        ...
        // If the link belongs to the current player
        if (link->permission.getOwner()->getPlayerNumber() - 1 == playerView) {
          // If the link is a data link
          if (link->getType() == 0) {
            // Set the color to seagreen2
            color = Xwindow::Seagreen2;
          } else {
            // Set the color to brown1
            color = Xwindow::Brown1;
          }
        } else {
          // Set the color to gray50
  ...
  // If the item is a link
    ...
    if (link->permission.getOwner()->getPlayerNumber() - 1 == playerView) {
      ...
      if (link->getType() == 0) {
        ...
        // If the link is a virus link
        window.fillRectangle(xPos + 1, yPos + 1, cellSize - 2, cellSize - 2,
                             Xwindow::Brown1);
      }
    } else {
      // If the link is viewable by the current player
      if (link->permission.viewableBy(*curPlayer)) {
        ...
        // If the link is not viewable by the current player
        window.fillRectangle(xPos + 1, yPos + 1, cellSize - 2, cellSize - 2,
                             Xwindow::Gray50);
      }
    }
